...
function validate()
{
	...
	else
	{
		...
		if (fileInput.files.length > 0) {
			data.append('poster', fileInput.files[0]);
		}
		data.append('agree_terms', true);

		fetch('http://127.0.0.1:8000/api/movies/submit/', {
			method: 'POST',
			body: data
		}).then(res => res.json()).then(res => {
			alert("Movie submitted successfully! Our moderators will review it.");
			form.reset();
		}).catch(err => {
